...
- auth-service의 security 의존은 `platform-runtime-bom`, `platform-security-starter`, `platform-security-governance-bridge`로 제한합니다.
- security audit event를 governance audit-log로 연결할 때만 `platform-integrations`의 `platform-security-governance-bridge`를 사용합니다.
- `AuthPlatformIssuerAdaptersConfiguration`과 `AuthPlatformOperationalConfiguration`이 `PlatformTokenIssuerPort`, `PlatformSessionIssuerPort`, `PlatformSessionSupportFactory`, `PlatformRateLimitPort`를 직접 제공합니다.
...
auth-service 도메인 코드는 governance adapter/core/engine 구현 타입을 직접 소비하지 않고, `platform-governance` public contract만 사용합니다.
...
- `AuthAuditLogService`는 `GovernanceAuditRecorder`를 주입받아 감사 이벤트 기록 경계를 담당합니다.
- 로그인, 로그아웃, internal account 변경 시점은 auth-service 도메인 코드에서 명시합니다.
- 감사 entry 변환과 category/message/attribute 결정은 auth-service 도메인 코드가 소유합니다.
