...
Auth-server는 `platform-security`, `platform-governance`, `platform-integrations`를 공통 경계로 사용합니다.
...
- Auth-server의 `platform-security` 직접 의존은 `platform-security-bom`과 `platform-security-starter`로 제한합니다.
