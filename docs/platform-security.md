...
`platform-security`의 주요 모듈 역할은 아래와 같습니다.

| 모듈 | 역할 |
| --- | --- |
| `platform-security-policy` | boundary, auth-mode 정책 모델 |
| `platform-security-auth` | 인증 수단별 capability와 resolver 조립 |
| `platform-security-web` | servlet/webflux 요청을 platform security 요청으로 변환 |
| `platform-security-autoconfigure` | Spring Boot bean 자동 설정 |
| `platform-security-starter` | 소비 서비스 진입점 |

## Auth-server 소비 기준

Auth-server는 1계층 auth 구현 타입을 직접 소비하지 않습니다.
...
| 인증 결과 | `io.github.jho951.platform.security.api.SecurityContext` |
| servlet filter | `io.github.jho951.platform.security.web.PlatformSecurityServletFilter` |
| web adapter 보조 타입 | `io.github.jho951.platform.security.web.*` |
...
- `SecurityConfig`는 `PlatformSecurityServletFilter`를 Spring Security filter chain에 연결합니다.
- `PlatformSecurityAuthConfig`는 Auth-server 도메인 token/session 저장소를 읽어 `SecurityContext`를 만듭니다.
...
1. `platform-security-bom:1.0.3`과 `platform-security-starter:1.0.3`을 사용합니다.
2. Auth-server 코드에서 `com.auth.*` 1계층 타입을 직접 import하지 않습니다.
3. `SecurityContextResolver`는 Auth-server 도메인 token/session 저장소를 읽어 `SecurityContext`를 반환합니다.
